          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {fleet.map((vehicle) => (
              <article key={vehicle.name} className="overflow-hidden rounded-md border border-zinc-200 bg-white shadow-sm">
                <div className="mountain-panel flex h-56 items-end p-5 text-white">
                  <div>
                    <CarFront className="mb-3 text-red-200" />
                    <h2 className="text-3xl font-black">{vehicle.name}</h2>
                  </div>
                </div>
                <div className="p-5">
                  <p className="text-sm leading-6 text-zinc-600">{vehicle.description}</p>
